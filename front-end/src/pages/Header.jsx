    ...
    <div className="navbar bg-green-500 h-20 mx-auto flex items-center justify-between">
      {user ? (
        <div className="flex items-center">
          <Link to={`/userprofile/${user.id}`} className="btn bg-green-600 text-white shadow-md hover:bg-white hover:text-green-500 pl-2 ml-3 mt-2 w-64 h-10">ยินดีต้อนรับ {user.username}</Link>
          <button onClick={handleLogout} className="btn bg-white text-red-600 border-[2px] border-red-600 shadow-md hover:bg-red-500 hover:text-white pl-2 ml-3 mt-2 w-64 h-10">ออกจากระบบ</button>
        </div>
      ) : (
